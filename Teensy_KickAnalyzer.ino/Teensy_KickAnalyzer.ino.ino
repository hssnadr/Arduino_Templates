...
void loop() {
  ...
  bool available_ = fft1024.available();
  int curVal_ = 0;
  ...
  if (available_) {
    ...
    if (indxWindw < WINDOW) {
      ...
      curMeanWindow = (curMinWindow + curMaxWindow) * 0.7;
    ...
    if (curVal_ > curMeanWindow && oldMean < curMeanWindow) {
      Serial.print(14);
    } else {
      Serial.print(0);
    }
